    ...
    stages {
        ...
        stage("push") {
            steps {
                withCredentials(
                    [
                        usernamePassword(usernameVariable: 'LOGIN', passwordVariable: 'PASSWORD', credentialsId: 'dockerhub')
                        ]
                    ) {
        ...
        stage("deploy") {
            steps {
                withCredentials(
                    ...
                    sh 'scp -o StrictHostKeyChecking=no -i "${KEY_FILE}" goltsev.prod.mshp-devops.com.conf ${USERNAME}@${SERVER_ADDRESS}:nginx'
